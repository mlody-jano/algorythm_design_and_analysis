 ...
 */
Game::Game(int boardSize) : board_(boardSize), turn_(Turn::Player), maxDepth_(depthForSize(boardSize)) {}
 ...
 */
void Game::reset(int boardSize) {
    board_    = Board(boardSize);
    turn_     = Turn::Player;
    maxDepth_ = depthForSize(boardSize);
}
